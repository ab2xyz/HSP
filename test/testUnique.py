...
dataSig_Bkg=torch.tensor([5,1,6,4,4,8,9,5,7,3,4])    # Origin values
uidBkg=torch.tensor([2,3,3,4,4,5,5,2,2,3,3])     # origin keys
# idx=torch.range(0,uidBkg.shape[0]-1).long()




dataSig_BkgArgsort=torch.argsort(dataSig_Bkg, dim=-1, descending=True)    # sort : Origin values   -  from larger to less
